...
const _displayNextItem = (results, resultCount, searchTerms, highlightTerms) => {
  ...
  if (results.length) {
    ...
    setTimeout(() => _displayNextItem(results, resultCount, searchTerms, highlightTerms), 5)
  }
      ...
      if (word.length > 2) {
        ...
        if (!terms.hasOwnProperty(word)) {
          Object.keys(terms).forEach((term) => {
            if (term.match(escapedWord)) arr.push({ files: terms[term], score: Scorer.partialTerm })
          })
        }
